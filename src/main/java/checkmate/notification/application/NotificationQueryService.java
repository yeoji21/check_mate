...
public class NotificationQueryService {
    ...
    public NotificationAttributeInfoResult findGoalCompleteNotifications(long userId) {
        List<NotificationAttributeInfo> notifications = notificationRepository.findUncheckedReceivers(
                userId, NotificationType.COMPLETE_GOAL)
            .stream()
            .map(receiver -> {
                receiver.read();
                return mapper.toInfo(receiver.getNotification());
    ...
    private void read(NotificationReceiver receiver) {
        if (receiver.getNotification().getType() != NotificationType.INVITE_SEND) {
            receiver.read();
        }
    }
}
