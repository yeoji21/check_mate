...
public class QueryPerformanceTest {
    ...
    void findTodayGoalInfoDtoList() throws Exception{
        ...
        List<TodayGoalInfo> result = queryFactory.select(new QTodayGoalInfo(goal.id, goal.category, goal.title, goal.weekDays,
                        new CaseBuilder().when(teamMate.lastUploadDay.eq(LocalDate.now())).then(true).otherwise(false)))
                .from(teamMate)
                .join(teamMate.goal, goal).on(goal.weekDays.weekDays.divide(WeekDayConverter.localDateToValue(LocalDate.now()))
...
//        List<Integer> right = WeekDayConverter.allCasesWith(LocalDate.now().getDayOfWeek().toString());
        ...
        List<TodayGoalInfo> slowResult = queryFactory.select(new QTodayGoalInfo(goal.id, goal.category, goal.title, goal.weekDays,
                        new CaseBuilder().when(teamMate.lastUploadDay.eq(LocalDate.now())).then(true).otherwise(false)))
                .from(teamMate)
                .join(teamMate.goal, goal).on(goal.weekDays.weekDays.in(right), goal.goalStatus.eq(GoalStatus.ONGOING))
                .where(teamMate.userId.eq(11L),
                        teamMate.teamMateStatus.eq(TeamMateStatus.ONGOING))
                .fetch();
        stopWatch.stop();
